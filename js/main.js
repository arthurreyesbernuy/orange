...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayCategories(categories) {
        ...
        categories.forEach(category => {
            ...
            categoryCard.innerHTML = `
                <img src="${category.image.replace('http://', 'http://')}" alt="${category.name}">
                <h3>${category.name}</h3>
            `;
    ...
    function displayProducts(products) {
        ...
        products.forEach(product => {
            ...
            productCard.innerHTML = `
                <img src="${product.image.replace('http://', 'https://')}" alt="${product.name}">
    ...
    async function placeOrder() {
        ...
        const fields = {
            ...
            "vads_cust_address": "av la paz 1564 Trujillo",
            "vads_cust_city": "Trujillo", // Estos datos deben venir del usuario
            ...
            "vads_cust_national_id": currentUser.numerodocumento, // Este dato debe venir del usuario
            "vads_cust_phone": currentUser.phone,
            "vads_cust_state": "Trujillo", // Estos datos deben venir del usuario
        ...
        try {            
            const signatureResponse = await getPaymentSignatureAPI(fields);

            if (!signatureResponse.success || !signatureResponse.signature) {
                alert('Error de seguridad: No se pudo generar la firma del pago.');
                return;
            }
            
            // --- PASO 3: Añade la firma recibida al resto de los campos ---
            fields["signature"] = signatureResponse.signature;

            // --- PASO 4: Construye el formulario final ---
            const paymentForm = document.getElementById('payment-form-campos');
            ...
            alert('Error al contactar al servidor para validar el pago.');
            console.error("Error getting signature:", error);
        }
        
        //fields["signature"] = calculateSignatureJS(fields, KEY);
